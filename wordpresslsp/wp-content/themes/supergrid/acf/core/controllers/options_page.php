...
class acf_options_page 
{
	...
	*/
	...
	function admin_menu() 
	{
		...
		// Parent
		$parent_page = add_theme_page($parent_title, $parent_menu, $defaults['capability'], $parent_slug, array($this, 'html'));	
		...
		if( !empty($defaults['pages']) )
		{
			foreach($defaults['pages'] as $c)
			{
				...
				$child_page = add_theme_page($parent_slug, $sub_title, $sub_title, $defaults['capability'], $sub_slug, array($this, 'html'));
